...
public class UsuarioService {
	...
	private void enviarEmailDeRecuperacaoDeSenha(String novaSenha, Usuario usuario) {
		...
		String texto = "<div style='border: 2px solid #ccc; padding: 40px; margin: 40px;'>" + "<center>"
				...
				+ novaSenha + "</strong></p>" + "<p>Att,</p>" + "<p>Equipe COTI Informática</p>" + "</center>"
				+ "</div>";
	...
	private <T> T getContentFromToken(String token, Function<Claims, T> claimsResolver) {
		final Claims claims = Jwts.parser().setSigningKey(JwtFilter.SECRET.getBytes()).parseClaimsJws(token).getBody();
